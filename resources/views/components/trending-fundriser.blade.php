    ...
    <div class="inset-0 container grid grid-cols-1">      
        <div class="row-start-2 col-start-1"
          ...
          <div class="relative grid grid-cols-1 text-white text-center justify-items-center md:mb-12 z-10">
              <h1 class="text-2xl sm:text-4xl">Our Ongoing Fundrisings</h1>
              <p class="text-xl sm:text-2xl text-gray-300">View the fundrisings that are most active right now</p>
          </div>
          <div class="grid grid-cols-1 grid-rows-1" x-data="carousel()" x-init="init()">  
            <div class="carousel col-start-1 row-start-1 z-20" x-ref="carousel">
              @foreach($fundrisings as $fundrising)
              <div class="w-80 bg-white shadow-md border border-gray-200 rounded-lg max-w-sm mb-5 mx-8">
                <a href="#">
                    <img class="rounded-t-lg w-full h-60" src="{{ $fundrising->getFirstMediaUrl('images', 'thumb') }}" alt="">
                </a>
                <div class="p-5">
                    <a href="#">
                        <h5 class="text-gray-900 font-bold text-2xl tracking-tight mb-2 line-clamp-2">{{ $fundrising->title }}</h5>
                    </a>
                    <div class="flex items-center">
                      <img class="h-8 w-8 rounded-full object-cover" src="{{ $fundrising->user->profile_photo_url }}" alt="{{ $fundrising->user->name }}" />
                      <span class="text-xs ml-2">by Test User</span>
                    </div>
                    <div class="relative pt-1 mt-2">
                      <span><strong></strong> <span class="text-gray-400">raised out of {{ $fundrising->target }}</span></span>
                    ...
                    <div class="flex mt-2">
                        <a data-layout="button" target="_blank" href="https://www.facebook.com/sharer/sharer.php?u=https://ummahaidsociety.com/{{ $fundrising->slug }}"
